...
var STORAGE_KEY = "planker-plaster-edit-v1";
var LIMIT_SHEETS = 80;
...
function packJob(job) {
  ...
  job.desired.forEach(function (panel) {
    for (var n = 0; n < panel.count; n++) {
      panels.push({w: panel.w, h: panel.h, grain: !!panel.grain, i: pieceI});
      pieceI += 1;
    }
  });
  ...
  panels.forEach(function (panel) {
    ...
    if (!best) {
      unplaced.push({w: panel.w, h: panel.h, grain: panel.grain});
      return;
    }
...
function shopText(plan, job, custom) {
  ...
  if (custom) banner = "CUSTOM SIZES — edited on this phone, not the shipped example";
...
function colourMap(plan) {
  ...
  plan.sheets.forEach(function (sheet) {
    sheet.placements.forEach(function (panel) { take(sizeKey(panel)); });
  });
  plan.unplaced.forEach(function (panel) { take(sizeKey(panel)); });
  return map;
}
...
function canonical(job) {
  return JSON.stringify({
    ...
    desired: job.desired.map(function (row) { return [row.count, row.w, row.h, row.grain ? 1 : 0]; })
  });
}
...
function boardSvg(sheet, colours) {
  ...
  sheet.placements.forEach(function (panel) {
    var fill = colours[sizeKey(panel)] || "#d7c4a8";
...
function chipsHtml(sheet, colours) {
  var html = sheet.placements.map(function (panel, index) {
    ...
    return "<span class='chip'><i style='background:" + (colours[sizeKey(panel)] || "#d7c4a8") + "'></i>" +
      (index + 1) + " · " + panel.w + " × " + panel.h + note + "</span>";
  }).join("");
  sheet.waste.forEach(function (offcut) {
...
function legendHtml(plan, colours) {
  var seen = {};
  var items = "";
  function add(key, label) {
    if (seen[key]) return;
    seen[key] = true;
    items += "<li><i class='swatch' style='background:" + colours[key] + "'></i>" + esc(label) + "</li>";
  }
  plan.sheets.forEach(function (sheet) {
    sheet.placements.forEach(function (panel) {
      var name = mmSize(panel.source_w, panel.source_h);
      if (panel.grain) name += ", face locked";
      add(sizeKey(panel), name);
    });
  });
  plan.unplaced.forEach(function (panel) {
    var name = mmSize(panel.w, panel.h);
    if (panel.grain) name += ", face locked";
    add(sizeKey(panel), name);
  });
  items += "<li><i class='key off'></i> Offcut</li><li><i class='key kerf'></i> Kerf " + plan.kerf_mm + " mm</li>";
  return items;
}
...
function paint(job) {
  ...
  if (kicker) {
    kicker.textContent = mode === "custom" ? "Edited on this phone" : (mode === "example" ? "Example · synthetic job" : "Plasterboard");
  }
  if (banner) {
    banner.textContent = mode === "custom"
      ? "Edited on this phone. These sizes are not the shipped example, and they are not loaded from a job file. Restore example job puts the synthetic example back."
      : "EXAMPLE / SYNTHETIC — not a real job. The counts and sizes shipped with this page are made up.";
  }
  ...
  if (legend) legend.innerHTML = legendHtml(plan, colours);
  var cutNote = document.getElementById("cut-note");
  if (cutNote) {
    cutNote.textContent = "Each sheet is to scale with the others. Hatched areas are offcuts. The dark strip is the " +
...
function rowFields(kind, row) {
  ...
  return '<div class="edit-row"><label>Count <input inputmode="numeric" enterkeyhint="done" autocomplete="off" min="1" step="1" value="' +
    ...
    height + '"></label><button type="button" class="remove ghost">Remove</button>' + lock + "</div>";
}
