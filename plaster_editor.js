...
var roomViewState = {rx: -18, ry: 32, z: 0.15, px: 0, py: 0};
var roomPointers = {};
var roomPinched = false;
var EYE = {x: 1900, y: 1500, z: 1400};
...
function toCss(x, y, z) {
  return [x - EYE.x, EYE.y - y, EYE.z - z];
}
...
function boot() {
  ...
  if (resetBtn) {
    resetBtn.addEventListener("click", function () {
      roomViewState = {rx: -18, ry: 32, z: 0.15, px: 0, py: 0};
      applyRoomView();
    });
  }
  if (roomView) {
    ...
    roomView.addEventListener("pointermove", function (event) {
      ...
      if (ids.length >= 2) {
        ...
        if (nextZ < 0.04) nextZ = 0.04;
        if (nextZ > 0.4) nextZ = 0.4;
